  ...
  <v-card class="pa-2" outlined round>
    ...
    <v-card-text>
      <v-row>
        <v-col>
          <v-card class="pa-2">
            <v-select :items="fillerQueueStates" v-model="activeFillerQueueState" outlined :value="fillerQueueState"
                      label="FillerQueue staat"></v-select>
      ...
      <v-row>
        <v-col>
          <v-card class="pa-2">
            ...
            <v-select :items="stateItems" v-model="selectedState" :value="playbackState" outlined
                      label="Playback staat"></v-select>
            <div class="text-h6 ma-2">Max requests gebruiker</div>
            <v-select :disabled="playbackState === 0" :items="maxRequestItems" v-model="maxRequestAmount"
                      :value="maxUserRequestAmount" outlined label="Playback staat"></v-select>
          </v-card>
        </v-col>
      </v-row>
      <v-row>
...
export default class PlaybackSettingsDashboard extends Vue {
  ...
  onSelectedStateChanged(newValue: any, oldValue: any) {
    ...
    // @ts-ignore
    axios.put(`api/playback/mod/setPlaybackState?playbackState=${this.selectedState}`)
  ...
  async handleReset() {
    ...
    await axios.put(`api/playback/mod/reset?maxRequestAmount=${this.maxRequestAmount}&term=${this.selectedTerm}`);
  }
