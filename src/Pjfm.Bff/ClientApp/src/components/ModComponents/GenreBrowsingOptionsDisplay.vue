<template>
  <div>
    ...
    <div :class="`${currentSeedAmount <= 5? 'orange--text': 'red--text'}`">{{ currentSeedAmount }}/5 seeds gebruikt
    </div>
...
export default class GenreBrowsingOptionsDisplay extends Vue {
  ...
  created() {
    // @ts-ignore
    axios.get("api/playback/mod/spotifyGenres")
  ...
  get browserQueueSettings(): browserQueueSettings {
    ...
    if (this.currentQueueSettings === null) {
      ...
      if (settings.seedTracks.length > 0) {
        // @ts-ignore
        axios.get(`api/track/multiple?trackIds=${settings.seedTracks.join(",")}`)
  ...
  applySettings(): void {
    ...
    axios.post("api/playback/mod/browserQueueSettings", this.browserQueueSettings)
        .then((response: AxiosResponse) => {
