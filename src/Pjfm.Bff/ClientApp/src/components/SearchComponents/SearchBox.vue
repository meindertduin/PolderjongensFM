<template>
  <div>
    <v-alert v-if="userIsMod === false || this.$store.getters['userSettingsModule/getMakeRequestAsMod'] === false"
      ...
      Je hebt momenteel {{ userRequestedAmount.toString() }} / {{ maxRequestsPerUser }} van het maximaal aantal
      verzoekjes in de wachtrij staan.
    </v-alert>
    <v-card>
      <v-tabs
          v-model="tab"
            ...
            <div v-if="!isLoading">
              <v-expansion-panels accordion class="mb-5">
                <v-expansion-panel
                    ...
                    :key="i"
                >
                  <v-expansion-panel-header @click="togglePlaylistDialog(playlist.id, playlist.name)">{{ i + 1 }}.
                    {{ playlist.name }}
        ...
        <v-tab-item>
          <v-card flat>
            <v-card-text>
              ...
              <v-text-field prepend-icon="mdi-magnify" label="Zoek naar artiesten of nummers"
                            v-on:keyup="searchBarKeyUp($event)" v-model="query"></v-text-field>
