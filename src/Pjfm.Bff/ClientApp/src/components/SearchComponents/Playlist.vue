...
export default class Playlist extends Vue {
  ...
  private getPlaylist() {
    axios.get(`/api/playlist/tracks?playlistId=${this.playlistId}`).then((results: AxiosResponse) => {
  ...
  private getTopTracksPlaylist(term: number) {
    ...
    // @ts-ignore
    axios.get(`/api/playlist/top-tracks?term=${termString}`).then((trackResponse: AxiosResponse) => {
