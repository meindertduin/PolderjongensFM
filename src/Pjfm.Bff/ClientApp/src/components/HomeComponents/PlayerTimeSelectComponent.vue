<template>
  <v-card>
    ...
    <v-card-text>
      ...
      Als je tijdens het luisteren wilt dat PJFM stopt met het besturen van jouw account kan je op STOP onder in het
      scherm klikken.<br><br>
      <v-spacer></v-spacer>
      <v-row justify="center">
        <v-col class="col-8 col-md-10">
          <v-select label="afpseel apparaat" outlined v-model="selectedDevice" :items="userDevices"
...
export default class PlayerTimeSelectComponent extends Vue {
  ...
  created() {
    axios.get('/api/playback/devices')
        .then(({data}: { data: playbackDevice[] | undefined }) => {
          if (data != null) {
